...
public interface GenJrnlRepository extends JpaRepository<Journal, Long> {
	...
	@Query(value = "   SELECT    JournalID,JournalNo,DATE( ClientCDate1 ) , DebitAmount, CreditAmount,     DrAccName, CrAccName,invoicename,personame, IF(adjbalflag=1,'GenAcc', 'AdjustAcc')  FROM medc_accounts.medc_journal    where  LocRefID=?2  and  LocName=?1  and JrnlType=1  and Status!=5 ", nativeQuery = true)
	List viewGenJournalAll(int lcrnm, int lcrid);

	@Query(value = "     SELECT   pt.JournalID,pt. JournalNo , DATE( pt.ClientCDate1 )  ,pt. debitaccount,pt. creditaccount,    pt. debitamount,pt. creditamount,pt.draccname ,pt.craccname, pt. invoiceno,    pt. invoicebalamt   ,pt.ClientCDate1  as cldate ,   pt.CashFlag, pt.JrnlName,pt. bulkflag,     pt.delflag, pt.personid, pt.persontype, pt.invoicetype, pt.paymenttype,     pt.ptrefno,pt.invoicename,pt.personame,pt.jrnltype  FROM medc_accounts.medc_journal  pt    where   pt.JournalID=?3   and  locrefid=?2  and  LocName=?1  and JrnlType=1   and status!=5 ", nativeQuery = true)
